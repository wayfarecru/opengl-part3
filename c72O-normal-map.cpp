...
const unsigned int WIN_W = 700; // window size in pixels, (Width, Height)
const unsigned int WIN_H = 700;
const unsigned int WIN_X = 100; // window position in pixels, (X, Y)
const unsigned int WIN_Y = 100;

const char* vertFileName = "c72-normal-map.vert";
const char* fragFileName = "c72-normal-map.frag";
...
glm::vec3 normalCoordSphere[10240];
glm::vec3 tangentSphere[10240];

GLuint texObj = 0;
GLuint texNormal = 0;

#include "c72-brick-map.cpp"
#include "c72-normal-map.cpp"
...
void initTex(void) {
	...
	if (glewIsSupported("GL_VERSION_4_5") && (use_new_tex_syntax == true)) {
		...
		glTextureStorage2D(texObj, 1, GL_RGBA8, BRICK_MAP_WIDTH, BRICK_MAP_HEIGHT);
		glTextureSubImage3D(texObj, 0, 0, 0, 0, BRICK_MAP_WIDTH, BRICK_MAP_HEIGHT, 1,
		                    GL_RGBA, GL_UNSIGNED_BYTE, brick_map);
		glTextureSubImage3D(texObj, 0, 0, 0, 1, BRICK_MAP_WIDTH, BRICK_MAP_HEIGHT, 1,
		                    GL_RGBA, GL_UNSIGNED_BYTE, brick_map);
		glTextureSubImage3D(texObj, 0, 0, 0, 2, BRICK_MAP_WIDTH, BRICK_MAP_HEIGHT, 1,
		                    GL_RGBA, GL_UNSIGNED_BYTE, brick_map);
		glTextureSubImage3D(texObj, 0, 0, 0, 3, BRICK_MAP_WIDTH, BRICK_MAP_HEIGHT, 1,
		                    GL_RGBA, GL_UNSIGNED_BYTE, brick_map);
		glTextureSubImage3D(texObj, 0, 0, 0, 4, BRICK_MAP_WIDTH, BRICK_MAP_HEIGHT, 1,
		                    GL_RGBA, GL_UNSIGNED_BYTE, brick_map);
		glTextureSubImage3D(texObj, 0, 0, 0, 5, BRICK_MAP_WIDTH, BRICK_MAP_HEIGHT, 1,
		                    GL_RGBA, GL_UNSIGNED_BYTE, brick_map);
		...
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X, 0, GL_RGBA, BRICK_MAP_WIDTH, BRICK_MAP_HEIGHT, 0,
		             GL_RGBA, GL_UNSIGNED_BYTE, brick_map);
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + 1, 0, GL_RGBA, BRICK_MAP_WIDTH, BRICK_MAP_HEIGHT, 0,
		             GL_RGBA, GL_UNSIGNED_BYTE, brick_map);
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + 2, 0, GL_RGBA, BRICK_MAP_WIDTH, BRICK_MAP_HEIGHT, 0,
		             GL_RGBA, GL_UNSIGNED_BYTE, brick_map);
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + 3, 0, GL_RGBA, BRICK_MAP_WIDTH, BRICK_MAP_HEIGHT, 0,
		             GL_RGBA, GL_UNSIGNED_BYTE, brick_map);
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + 4, 0, GL_RGBA, BRICK_MAP_WIDTH, BRICK_MAP_HEIGHT, 0,
		             GL_RGBA, GL_UNSIGNED_BYTE, brick_map);
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + 5, 0, GL_RGBA, BRICK_MAP_WIDTH, BRICK_MAP_HEIGHT, 0,
		             GL_RGBA, GL_UNSIGNED_BYTE, brick_map);
		glGenerateMipmap(GL_TEXTURE_CUBE_MAP);
		glTexParameterf(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
		glTexParameterf(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	}
	// one more texture for the normal map
	glGenTextures(1, &texNormal);
	// bind the texture object to the texture unit #1
	glActiveTexture(GL_TEXTURE1); // select tex unit #0
	glBindTexture(GL_TEXTURE_CUBE_MAP, texNormal);
	// new for OpenGL 4.5+: load an image to the texture object
	if (glewIsSupported("GL_VERSION_4_5") && (use_new_tex_syntax == true)) {
		printf("using OpenGL 4.5 syntax\n");
		glTextureStorage2D(texNormal, 1, GL_RGBA8, NORMAL_MAP_WIDTH, NORMAL_MAP_HEIGHT);
		glTextureSubImage3D(texNormal, 0, 0, 0, 0, NORMAL_MAP_WIDTH, NORMAL_MAP_HEIGHT, 1,
		                    GL_RGBA, GL_UNSIGNED_BYTE, normal_map);
		glTextureSubImage3D(texNormal, 0, 0, 0, 1, NORMAL_MAP_WIDTH, NORMAL_MAP_HEIGHT, 1,
		                    GL_RGBA, GL_UNSIGNED_BYTE, normal_map);
		glTextureSubImage3D(texNormal, 0, 0, 0, 2, NORMAL_MAP_WIDTH, NORMAL_MAP_HEIGHT, 1,
		                    GL_RGBA, GL_UNSIGNED_BYTE, normal_map);
		glTextureSubImage3D(texNormal, 0, 0, 0, 3, NORMAL_MAP_WIDTH, NORMAL_MAP_HEIGHT, 1,
		                    GL_RGBA, GL_UNSIGNED_BYTE, normal_map);
		glTextureSubImage3D(texNormal, 0, 0, 0, 4, NORMAL_MAP_WIDTH, NORMAL_MAP_HEIGHT, 1,
		                    GL_RGBA, GL_UNSIGNED_BYTE, normal_map);
		glTextureSubImage3D(texNormal, 0, 0, 0, 5, NORMAL_MAP_WIDTH, NORMAL_MAP_HEIGHT, 1,
		                    GL_RGBA, GL_UNSIGNED_BYTE, normal_map);
		glGenerateTextureMipmap(texNormal);
		glTextureParameterf(texNormal, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
		glTextureParameterf(texNormal, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	} else { // original style: load an image to the texture unit (and its associated texture object)
		printf("using OpenGL 3.3+ syntax\n");
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X, 0, GL_RGBA, NORMAL_MAP_WIDTH, NORMAL_MAP_HEIGHT, 0,
		             GL_RGBA, GL_UNSIGNED_BYTE, normal_map);
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + 1, 0, GL_RGBA, NORMAL_MAP_WIDTH, NORMAL_MAP_HEIGHT, 0,
		             GL_RGBA, GL_UNSIGNED_BYTE, normal_map);
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + 2, 0, GL_RGBA, NORMAL_MAP_WIDTH, NORMAL_MAP_HEIGHT, 0,
		             GL_RGBA, GL_UNSIGNED_BYTE, normal_map);
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + 3, 0, GL_RGBA, NORMAL_MAP_WIDTH, NORMAL_MAP_HEIGHT, 0,
		             GL_RGBA, GL_UNSIGNED_BYTE, normal_map);
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + 4, 0, GL_RGBA, NORMAL_MAP_WIDTH, NORMAL_MAP_HEIGHT, 0,
		             GL_RGBA, GL_UNSIGNED_BYTE, normal_map);
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + 5, 0, GL_RGBA, NORMAL_MAP_WIDTH, NORMAL_MAP_HEIGHT, 0,
		             GL_RGBA, GL_UNSIGNED_BYTE, normal_map);
		glGenerateMipmap(GL_TEXTURE_CUBE_MAP);
		glTexParameterf(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
		glTexParameterf(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
...
void triangles(int level, const glm::vec3 a, const glm::vec3 b, const glm::vec3 c) {
	...
	if (level <= 0) {
		...
		// normal map coordinate : currently, set as same as texCoord
		normalCoordSphere[numVertSphere] = a;
		normalCoordSphere[numVertSphere + 1] = b;
		normalCoordSphere[numVertSphere + 2] = c;
		// tangent vector for each coord
		tangentSphere[numVertSphere] = glm::cross(glm::vec3(0, 0, 1), a);
		tangentSphere[numVertSphere + 1] = glm::cross(glm::vec3(0, 0, 1), b);
		tangentSphere[numVertSphere + 2] = glm::cross(glm::vec3(0, 0, 1), c);
		// done
		numVertSphere += 3;
	} else {
...
void keyFunc(GLFWwindow* window, int key, int scancode, int action, int mods) {
	...
	case GLFW_KEY_2:
		if (glewIsSupported("GL_VERSION_4_5") && (use_new_tex_syntax == true)) {
			...
			glTextureParameterf(texNormal, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
			glTextureParameterf(texNormal, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
		} else {
			glActiveTexture(GL_TEXTURE0);
	...
	case GLFW_KEY_3:
		if (glewIsSupported("GL_VERSION_4_5") && (use_new_tex_syntax == true)) {
			...
			glTextureParameterf(texNormal, GL_TEXTURE_MIN_FILTER, GL_NEAREST_MIPMAP_NEAREST);
		} else {
			glActiveTexture(GL_TEXTURE0);
	...
	case GLFW_KEY_4:
		if (glewIsSupported("GL_VERSION_4_5") && (use_new_tex_syntax == true)) {
			...
			glTextureParameterf(texNormal, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_NEAREST);
		} else {
			glActiveTexture(GL_TEXTURE0);
	...
	case GLFW_KEY_5:
		if (glewIsSupported("GL_VERSION_4_5") && (use_new_tex_syntax == true)) {
			...
			glTextureParameterf(texNormal, GL_TEXTURE_MIN_FILTER, GL_NEAREST_MIPMAP_LINEAR);
		} else {
			glActiveTexture(GL_TEXTURE0);
	...
	case GLFW_KEY_6:
		if (glewIsSupported("GL_VERSION_4_5") && (use_new_tex_syntax == true)) {
			...
			glTextureParameterf(texNormal, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
		} else {
			glActiveTexture(GL_TEXTURE0);
...
void drawFunc(void) {
	...
	glVertexAttribPointer(locNormalCoord, 3, GL_FLOAT, GL_FALSE, 0, glm::value_ptr(normalCoordSphere[0]));
	GLuint locTangent = glGetAttribLocation(prog, "aTangent");
	glEnableVertexAttribArray(locTangent);
	glVertexAttribPointer(locTangent, 3, GL_FLOAT, GL_FALSE, 0, glm::value_ptr(tangentSphere[0]));
	GLuint locTexSampler = glGetUniformLocation(prog, "texSampler");
	glUniform1i(locTexSampler, 0);
	GLuint locNormalSampler = glGetUniformLocation(prog, "normalSampler");

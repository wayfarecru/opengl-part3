...
const char* vertFileName = "c82-dual-tran.vert";
const char* fragFileName = "c82-dual-tran.frag";
...
void initTex(void) {
	...
	if (glewIsSupported("GL_VERSION_4_5") && (use_new_tex_syntax == true)) {
		...
		glTextureSubImage2D(texObj[1], 0, 0, 0, LOGO_IMG_WIDTH, LOGO_IMG_HEIGHT, GL_RGBA, GL_UNSIGNED_BYTE, logo_img);
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, LOGO_IMG_WIDTH, LOGO_IMG_HEIGHT, 0,
		             GL_RGBA, GL_UNSIGNED_BYTE, logo_img);
		glGenerateMipmap(GL_TEXTURE_2D);
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	}
...
void drawFunc(GLFWwindow* window) {
	...
	// provide alpha blending factor
	system_clock::time_point curTime = system_clock::now();
	milliseconds curTimeMSEC = duration_cast<milliseconds>(curTime.time_since_epoch()); // in millisecond
	GLfloat alpha = (curTimeMSEC.count() % 4000) / 4000.0F; // 4 sec period
	GLuint locAlpha = glGetUniformLocation(prog, "alpha");

...
void drawFunc(void) {
	...
	// STENCIL WRITE stage: draw to the stencil buffer
	...
	// STENCIL USE stage: draw again, with highlight
	glEnable(GL_STENCIL_TEST);
	glStencilMask(0xFF);
	glStencilFunc(GL_EQUAL, 0x00, 0xFF); // if (0x00 == stencil value), stencil passed
	glStencilOp(GL_KEEP, GL_KEEP, GL_KEEP); // READ-ONLY: do not update the stencil buffer
	// draw the cube: highlighted !
	glUniform1i(locHighlight, 1); // highlight on
	glVertexAttribPointer(locPos, 4, GL_FLOAT, GL_FALSE, 8 * sizeof(GLfloat), glm::value_ptr(vertCube[0]));
	glVertexAttribPointer(locColor, 4, GL_FLOAT, GL_FALSE, 8 * sizeof(GLfloat), glm::value_ptr(vertCube[1]));
	glm::mat4 mat = glm::scale( matCube, glm::vec3(1.1F, 1.1F, 1.1F));
	glUniformMatrix4fv(locModel, 1, GL_FALSE, glm::value_ptr(mat));
	glDrawArrays(GL_TRIANGLES, 0, 36); // 36 vertices
	glUniform1i(locHighlight, 0); // highlight off
	// stencil buffer off

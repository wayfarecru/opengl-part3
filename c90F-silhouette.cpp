...
glm::vec4 vertPanel[] = { // 2 * 3 = 6 (vertices + color)
	// face 0: v0-v1-v2, black, CCW
	{ -0.5F, -0.4F, 0.7F, 1.0F }, { 0.0F, 0.0F, 0.0F, 1.0F, }, // v0
	{ +0.5F, -0.4F, 0.7F, 1.0F }, { 0.0F, 0.0F, 0.0F, 1.0F, }, // v1
	{ -0.5F, +0.4F, 0.7F, 1.0F }, { 0.0F, 0.0F, 0.0F, 1.0F, }, // v2
	// face 1: v2-v1-v3, black, CCW
	{ -0.5F, +0.4F, 0.7F, 1.0F }, { 0.0F, 0.0F, 0.0F, 1.0F, }, // v2
	{ +0.5F, -0.4F, 0.7F, 1.0F }, { 0.0F, 0.0F, 0.0F, 1.0F, }, // v1
	{ +0.5F, +0.4F, 0.7F, 1.0F }, { 0.0F, 0.0F, 0.0F, 1.0F, }, // v3
};
...
void drawFunc(void) {
	...
	// STENCIL WRITE stage: draw objects 4 times
	glEnable(GL_STENCIL_TEST);
	glStencilMask(0xFF);
	glStencilFunc(GL_ALWAYS, 0x01, 0xFF);
	glStencilOp(GL_KEEP, GL_KEEP, GL_INCR); // if depth test passed, increase stencil value
	// draw 4 times for slightly-moved positions
	glColorMask(false, false, false, false);
	glDisable(GL_DEPTH_TEST);
	glViewport(+0, +2, WIN_W, WIN_H);
	drawObjects(locPos, locColor, locModel);
	glViewport(+0, -2, WIN_W, WIN_H);
	drawObjects(locPos, locColor, locModel);
	glViewport(+2, +0, WIN_W, WIN_H);
	drawObjects(locPos, locColor, locModel);
	glViewport(-2, +0, WIN_W, WIN_H);
	drawObjects(locPos, locColor, locModel);
	glColorMask(true, true, true, true);
	glEnable(GL_DEPTH_TEST);
	// now, draw the object at the exact position
	glViewport(0, 0, WIN_W, WIN_H);
	glStencilFunc(GL_ALWAYS, 0x00, 0xFF);
	glStencilOp(GL_KEEP, GL_KEEP, GL_REPLACE); // if depth test passed, clear the stencil value with 0x00
	drawObjects(locPos, locColor, locModel);
	// STENCIL USE stage: draw big rectangle over the objects
	glEnable(GL_STENCIL_TEST);
	glStencilMask(0xFF);
	glStencilFunc(GL_LESS, 0x00, 0xFF); // if (0 < stencil value), stencil passed
	glStencilOp(GL_KEEP, GL_KEEP, GL_KEEP); // READ-ONLY: do not update the stencil buffer
	// draw the black panel
	glVertexAttribPointer(locPos, 4, GL_FLOAT, GL_FALSE, 8 * sizeof(GLfloat), glm::value_ptr(vertPanel[0]));
	glVertexAttribPointer(locColor, 4, GL_FLOAT, GL_FALSE, 8 * sizeof(GLfloat), glm::value_ptr(vertPanel[1]));

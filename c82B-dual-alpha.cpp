...
const char* vertFileName = "c82-dual-alpha.vert";
const char* fragFileName = "c82-dual-alpha.frag";
...
#include "c82-logo-img.cpp"

GLubyte logo_alpha[LOGO_IMG_HEIGHT * LOGO_IMG_WIDTH * 4] = { 0 };
...
void make_logo_alpha(void) {
	for (unsigned int i = 0; i < LOGO_IMG_HEIGHT * LOGO_IMG_WIDTH; ++i) {
		GLubyte r = logo_img[4 * i];
		GLubyte g = logo_img[4 * i + 1];
		GLubyte b = logo_img[4 * i + 2];
		GLubyte a = logo_img[4 * i + 3];
		if (r == 0x9f && g == 0xfc && b == 0xfd) a = 0x00; // background color
		else if (r == 0x00 && g == 0x00 && b == 0x00) a = 0xff; // black foreground
		else if (r == 0xa3 && g == 0x49 && b == 0xa4) a = 0xff; // magenta foreground
		else if (r == 0x3f && g == 0x48 && b == 0xcc) a = 0xff; // cyan foreground
		else a = 0x7f; // 50% alpha value
		logo_alpha[4 * i] = r;
		logo_alpha[4 * i + 1] = g;
		logo_alpha[4 * i + 2] = b;
		logo_alpha[4 * i + 3] = a;
...
void initTex(void) {
	...
	if (glewIsSupported("GL_VERSION_4_5") && (use_new_tex_syntax == true)) {
		...
		glTextureStorage2D(texObj[0], 1, GL_RGBA8, ROSE_IMG_WIDTH, ROSE_IMG_HEIGHT);
		glTextureSubImage2D(texObj[0], 0, 0, 0, ROSE_IMG_WIDTH, ROSE_IMG_HEIGHT, GL_RGBA, GL_UNSIGNED_BYTE, rose_img);
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, ROSE_IMG_WIDTH, ROSE_IMG_HEIGHT, 0,
		             GL_RGBA, GL_UNSIGNED_BYTE, rose_img);
	...
	if (glewIsSupported("GL_VERSION_4_5") && (use_new_tex_syntax == true)) {
		printf("using OpenGL 4.5 syntax\n");
		glTextureStorage2D(texObj[1], 1, GL_RGBA8, LOGO_IMG_WIDTH, LOGO_IMG_HEIGHT);
		glTextureSubImage2D(texObj[1], 0, 0, 0, LOGO_IMG_WIDTH, LOGO_IMG_HEIGHT, GL_RGBA, GL_UNSIGNED_BYTE, logo_alpha);
		...
		printf("using OpenGL 3.3+ syntax\n");
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, LOGO_IMG_WIDTH, LOGO_IMG_HEIGHT, 0,
		             GL_RGBA, GL_UNSIGNED_BYTE, logo_alpha);
		glGenerateMipmap(GL_TEXTURE_2D);
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	}

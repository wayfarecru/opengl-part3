...
glm::vec4 vertBluePanel[] = { // 2 * 3 = 6 (vertices + color)
	// face 0: v0-v1-v2, green, CCW
	{ 0.4F, -0.3F, -1.0F, 1.0F }, { 0.3F, 0.3F, 1.0F, 1.0F, }, // v0
	{ 1.0F, -0.3F, -0.4F, 1.0F }, { 0.3F, 0.3F, 1.0F, 1.0F, }, // v1
	{ 0.4F, +0.3F, -1.0F, 1.0F }, { 0.3F, 0.3F, 1.0F, 1.0F, }, // v2
	// face 1: v2-v1-v3, green, CCW
	{ 0.4F, +0.3F, -1.0F, 1.0F }, { 0.3F, 0.3F, 1.0F, 1.0F, }, // v2
	{ 1.0F, -0.3F, -0.4F, 1.0F }, { 0.3F, 0.3F, 1.0F, 1.0F, }, // v1
	{ 1.0F, +0.3F, -0.4F, 1.0F }, { 0.3F, 0.3F, 1.0F, 1.0F, }, // v3
};
glm::vec4 vertCyanPanel[] = { // 2 * 3 = 6 (vertices + color)
	// face 0: v0-v1-v2, yellow, CCW
	{ 0.3F, -0.4F, -1.1F, 1.0F }, { 0.3F, 0.6F, 0.6F, 1.0F, }, // v0
	{ 1.1F, -0.4F, -0.3F, 1.0F }, { 0.3F, 0.6F, 0.6F, 1.0F, }, // v1
	{ 0.3F, +0.4F, -1.1F, 1.0F }, { 0.3F, 0.6F, 0.6F, 1.0F, }, // v2
	// face 1: v2-v1-v3, yellow, CCW
	{ 0.3F, +0.4F, -1.1F, 1.0F }, { 0.3F, 0.6F, 0.6F, 1.0F, }, // v2
	{ 1.1F, -0.4F, -0.3F, 1.0F }, { 0.3F, 0.6F, 0.6F, 1.0F, }, // v1
	{ 1.1F, +0.4F, -0.3F, 1.0F }, { 0.3F, 0.6F, 0.6F, 1.0F, }, // v3
};
glm::mat4 matPanel = glm::mat4( 1.0F );
...
void drawObjects(GLuint locPos, GLuint locColor, GLuint locModel) {
	// draw the cube
...
void drawReflectedObjects(GLuint locPos, GLuint locColor, GLuint locModel) {
	static bool ref_calculated = false;
	static glm::mat4 ref;
	// reflection matrix calculation
	if (ref_calculated == false) {
		glm::vec3 p0(vertBluePanel[0].x, vertBluePanel[0].y, vertBluePanel[0].z);
		glm::vec3 p1(vertBluePanel[2].x, vertBluePanel[2].y, vertBluePanel[2].z);
		glm::vec3 p2(vertBluePanel[4].x, vertBluePanel[4].y, vertBluePanel[4].z);
		glm::vec3 v = glm::normalize(glm::cross(p1 - p0, p2 - p0));
		...
		float dot = glm::dot(p0, v);
		ref = glm::mat4( // caution: glm uses column-major format !
		          1 - 2 * v.x * v.x, -2 * v.x * v.y, -2 * v.x * v.z, 0,
		          -2 * v.x * v.y, 1 - 2 * v.y * v.y, -2 * v.y * v.z, 0,
		          -2 * v.x * v.z, -2 * v.y * v.z, 1 - 2 * v.z * v.z, 0,
		          2 * dot * v.x, 2 * dot * v.y, 2 * dot * v.z, 1
		      );
		ref_calculated = true;
	}
	// draw the cube
	glVertexAttribPointer(locPos, 4, GL_FLOAT, GL_FALSE, 8 * sizeof(GLfloat), glm::value_ptr(vertCube[0]));
	glVertexAttribPointer(locColor, 4, GL_FLOAT, GL_FALSE, 8 * sizeof(GLfloat), glm::value_ptr(vertCube[1]));
	glm::mat4 refCube = ref * matCube;
	glUniformMatrix4fv(locModel, 1, GL_FALSE, glm::value_ptr(refCube));
	glDrawArrays(GL_TRIANGLES, 0, 36); // 36 vertices
	// draw the pyramid
	glVertexAttribPointer(locPos, 4, GL_FLOAT, GL_FALSE, 8 * sizeof(GLfloat), glm::value_ptr(vertPyramid[0]));
	glVertexAttribPointer(locColor, 4, GL_FLOAT, GL_FALSE, 8 * sizeof(GLfloat), glm::value_ptr(vertPyramid[1]));
	glm::mat4 refPyramid = ref * matPyramid;
	glUniformMatrix4fv(locModel, 1, GL_FALSE, glm::value_ptr(refPyramid));
	glDrawArrays(GL_TRIANGLES, 0, 18); // 18 vertices
}
...
void drawFunc(void) {
	...
	glVertexAttribPointer(locPos, 4, GL_FLOAT, GL_FALSE, 8 * sizeof(GLfloat), glm::value_ptr(vertBluePanel[0]));
	glVertexAttribPointer(locColor, 4, GL_FLOAT, GL_FALSE, 8 * sizeof(GLfloat), glm::value_ptr(vertBluePanel[1]));
	glUniformMatrix4fv(locModel, 1, GL_FALSE, glm::value_ptr(matPanel));
	glDrawArrays(GL_TRIANGLES, 0, 6); // 6 vertices
	glColorMask(true, true, true, true);
	glDepthMask(true);
	// STENCIL USE: draw with reflected matrix
	glStencilFunc(GL_NOTEQUAL, 0x00, 0xFF);
	glStencilOp(GL_KEEP, GL_KEEP, GL_KEEP);
	drawReflectedObjects(locPos, locColor, locModel);
	// STENCIL USE: draw mirror frame
	glStencilFunc(GL_EQUAL, 0x00, 0xFF);
	glStencilOp(GL_KEEP, GL_KEEP, GL_KEEP);
	glVertexAttribPointer(locPos, 4, GL_FLOAT, GL_FALSE, 8 * sizeof(GLfloat), glm::value_ptr(vertCyanPanel[0]));
	glVertexAttribPointer(locColor, 4, GL_FLOAT, GL_FALSE, 8 * sizeof(GLfloat), glm::value_ptr(vertCyanPanel[1]));
	glUniformMatrix4fv(locModel, 1, GL_FALSE, glm::value_ptr(matPanel));
	glDrawArrays(GL_TRIANGLES, 0, 6); // 6 vertices
	// stencil buffer off
	glDisable(GL_STENCIL_TEST);
	// normal draw
	drawObjects(locPos, locColor, locModel);
#if 1

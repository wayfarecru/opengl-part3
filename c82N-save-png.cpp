...
void readFunc(void) {
	static int count = 0;
	if (count++ == 20) { // only once for 20th frame
		GLubyte val[WIN_W * WIN_H * 4];
		...
		glReadPixels(0, 0, WIN_W, WIN_H, GL_RGBA, GL_UNSIGNED_BYTE, val);
		glFinish();
		int ind = (10 * WIN_W + 10) * 4;
		printf("at (10,10): (%d %d %d %d)\n", val[ind], val[ind+1], val[ind+2], val[ind+3]);
		ind = (300 * WIN_W + 250) * 4;
		printf("at (250,300): (%d %d %d %d)\n", val[ind], val[ind+1], val[ind+2], val[ind+3]);
		printf("framebuffer read completed\n");
		FILE* fp = fopen("opengl-saved-image.png", "wb");
		svpng(fp, WIN_W, WIN_H, val, 1); // 500x500, RGBA format
		fclose(fp);

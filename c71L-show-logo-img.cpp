...
#include "c71-logo-img.cpp"

const GLfloat map_size_x = 0.9f;
const GLfloat map_size_y = map_size_x * (GLfloat)LOGO_IMG_HEIGHT / (GLfloat)LOGO_IMG_WIDTH;
...
void initTex(void) {
	...
	if (glewIsSupported("GL_VERSION_4_5") && (use_new_tex_syntax == true)) {
		...
		glTextureStorage2D(texObj, 1, GL_RGBA8, LOGO_IMG_WIDTH, LOGO_IMG_HEIGHT);
		glTextureSubImage2D(texObj, 0, 0, 0, LOGO_IMG_WIDTH, LOGO_IMG_HEIGHT, GL_RGBA, GL_UNSIGNED_BYTE, logo_img);

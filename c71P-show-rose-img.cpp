...
const char* vertFileName = "c71-single-tex.vert";
const char* fragFileName = "c71-single-tex.frag";
...
#include "c71-rose-img.cpp"

const GLfloat map_size_x = 0.9f;
const GLfloat map_size_y = map_size_x * (GLfloat)ROSE_IMG_HEIGHT / (GLfloat)ROSE_IMG_WIDTH;
...
void initTex(void) {
	...
	if (glewIsSupported("GL_VERSION_4_5") && (use_new_tex_syntax == true)) {
		...
		glTextureStorage2D(texObj, 1, GL_RGBA8, ROSE_IMG_WIDTH, ROSE_IMG_HEIGHT);
		glTextureSubImage2D(texObj, 0, 0, 0, ROSE_IMG_WIDTH, ROSE_IMG_HEIGHT, GL_RGBA, GL_UNSIGNED_BYTE, rose_img);
